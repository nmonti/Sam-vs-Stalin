...
'Space enables USA flag timer, timer disabled when flag hits Stalin or form
If tmrUSA.Enabled = False Then
    If KeyCode = vbKeySpace Then
        ...
        tmrUSA.Enabled = True
    End If
ElseIf tmrUSA.Enabled = True Then 'While timer enabled, space does nothing
...
'Loads frm1Player's Alaskan background image
frm1Player.Picture = LoadPicture(App.Path & "\alaska.jpg")
...
'Loads USSR flag where Stalin shoots
...
'disables Stalin direction timers on form load
...
'Stalin won't go past middle of form
...
'If intSamHP = 0, all timers diabled, loads frmLose and unloads frm1Player
If intSamHP = 0 Then
    tmrUSA.Enabled = False
    tmrUSSR.Enabled = False
    tmrStalinShoot.Enabled = False
